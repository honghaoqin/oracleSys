(function(d) {
	...
	d.widget("ech.multiselect", {
		options: {
			header: !0,
			height: 100,//展示的下拉框的div的高度
			minWidth: 100,//展示的下拉框的div的宽度
			classes: "",
			checkAllText: "Check all",
			uncheckAllText: "Uncheck all",
			noneSelectedText: "Select options",
			selectedText: "# selected",//默认显示 选择 x 项
			showText:false,//新增的参数 是否显示文本值到input框中
			showTextWidth:100,//显示的文本框的宽度
			selectedList: 0,
			show: "",
			hide: "",
			autoOpen: !1,
		...
		_create: function() {
			...
			this._isOpen = !1;
			a = (this.button = d('<button type="button" style="border:0px;" ><span class="ui-icon ui-icon-triangle-2-n-s" style="display:none;"></span></button>')).addClass("ui-multiselect ui-widget ui-state-default ui-corner-all").addClass(b.classes).attr({
				title: a.attr("title"),
				"aria-haspopup": !0,
				tabIndex: a.attr("tabIndex")
			}).insertAfter(a);
			var setInputName="";
			if(this.element.attr("name")!=null&& this.element.attr("name")!=undefined && this.element.attr("name")!=''){
				setInputName=" name= '"+this.element.attr("name")+"NAME' ";
			}
			var setInputId="";
			if(this.element.attr("id")!=null && this.element.attr("id")!=undefined && this.element.attr("id")!=''){
				setInputId=" id= '"+this.element.attr("id")+"_id' ";
			}
			//<input style='width:"+b.minWidth+"px' /> 原来是 <span/>
			(this.buttonlabel = d("<input "+setInputName+setInputId+" style='width:"+(b.showTextWidth)+"px;padding-left:0px;'/>")).html(b.noneSelectedText).appendTo(a);
		...
		update: function() {
			var na = this.options;
			if(na!=null && na.showText==true){
				b = this.labels.find("input"),
				c = b.filter("[checked]");
				//smhbug  只有一个的时候c为空
				a = 0 === e ?a.noneSelectedText :c.map(function() {
					return d(this).next().text()
				}).get().join(", ");
				this.buttonlabel.width(this.minWidth);
			    this.buttonlabel.attr("title",a);
				this.buttonlabel.val(a);//原来是this.buttonlabel.html(a);
			}else{
			var a = this.options,
				b = this.labels.find("input"),
				c = b.filter("[checked]"),

...
<div class="pnlForm" id="pnlForm_list">
	<div class="pnlHead">
		<div class="pnlHead_L">
			<div class="pnlHead_C">
				<div class="pnlTitle">
					<a class="switchBtn" onClick="javascript:sHlay(this,'hidden');"></a>定时任务列表
				</div>
	...
	<div class="pnlBody">
		<div class="pnlContent">
			...
			<form name="listForm" id="listForm"
				action="${ctx}/sys/Job/loadList.do" method="post">
				<!-- 隐含条件字段开始 -->
						...
						<s:iterator value="resultList" id="row" status="st">
							<tr <s:if test="#st.index%2!=0">class="oddRow"</s:if>
								...
								<td>${page.firstResult + st.index+1}</td>
									<td title="${row.DES}">${row.DES}</td>
								<td  onclick="loadView('${ctx}','${row.JOB_ID}')"
									style="cursor: pointer;">${row.STATE}</td>
								<td  onclick="loadView('${ctx}','${row.JOB_ID}')"
									style="cursor: pointer;">${row.START_TIME}</td>
								<td  onclick="loadView('${ctx}','${row.JOB_ID}')"
									style="cursor: pointer;">${row.END_TIME}</td>
								<td  onclick="loadView('${ctx}','${row.JOB_ID}')"
									style="cursor: pointer;">${row.NEXTFIRETIME}</td>
								<td  onclick="loadView('${ctx}','${row.JOB_ID}')"
									style="cursor: pointer;">${row.ISSTART}</td>
							</tr>
						</s:iterator>
					</s:if>
					<s:else>

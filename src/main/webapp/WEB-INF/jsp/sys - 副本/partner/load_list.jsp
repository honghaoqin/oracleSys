...
<div class="pnlForm" id="pnlForm_list">
	<div class="pnlHead">
		<div class="pnlHead_L">
			<div class="pnlHead_C">
				<div class="pnlTitle">
					<a class="switchBtn" onClick="javascript:sHlay(this,'hidden');"></a>伙伴列表
				</div>
	...
	<div class="pnlBody">
		<div class="pnlContent">
			...
			<form name="listForm" id="listForm"
				action="${ctx}/sys/Partner/loadList.do" method="post">
				<!-- 隐含条件字段开始 -->
		...
		<!-- UI Structure Begin "信息列表数据" -->
		 <s:if test="resultList.size!=0">
		    <s:iterator value="resultList" id="row" status="st">
		     <tr <s:if test="#st.index%2!=0">class="oddRow"</s:if><s:else>class="evenRow"</s:else>>
			<td align="center">
				<input type="checkbox" class="IDS" value="${row.PARTNER_ID}" 
				id="ID_${row.PARTNER_ID}">
			</td>
	      <td>
				${page.firstResult + st.index+1}
		</td>
			<td onclick="gotoView('${ctx}','${row.PARTNER_ID}')" style="cursor:pointer;" >
		${row.NAME}
		</td>
			<td onclick="gotoView('${ctx}','${row.PARTNER_ID}')" style="cursor:pointer;" >
		${row.PHONE}

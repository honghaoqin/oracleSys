...
<body>
	<nav class="breadcrumb">
		...
		系统管理 <span class="c-gray en">&gt;</span>登录日志 <a
	...
	<form name="listForm" id="listForm" action="${ctx}/sys/LoginLog/list.do"
		method="post">
		<!-- 隐含条件字段开始 -->
				...
				<table
					...
					<thead>
						<tr class="text-c">
							...
							<td>IP地址</td>
							<td class="hidden-480">浏览器</td>
							<td>登录时间</td>
							<td class="hidden-480">退出时间</td>
						...
						<s:if test="resultList.size!=0">
							<s:iterator value="resultList" id="row" status="st">
								<tr class="text-c">
									...
									<td>${row.IP}</td>
									<td class="hidden-480">${row.BROWSER}</td>
									<td><s:date name="LOGIN_TIME"
											format="yyyy-MM-dd HH:mm:ss" /></td>
									<td class="hidden-480"><s:date name="LOGOUT_TIME"
											format="yyyy-MM-dd HH:mm:ss" /></td>
								</tr>
							</s:iterator>
						</s:if>

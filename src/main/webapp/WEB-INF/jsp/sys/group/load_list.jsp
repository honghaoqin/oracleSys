...
<div class="pnlForm" id="pnlForm_list">
	<div class="pnlHead">
		<div class="pnlHead_L">
			<div class="pnlHead_C">
				<div class="pnlTitle">
					<a class="switchBtn" onClick="javascript:sHlay(this,'hidden');"></a>用户组列表
				</div>
						...
						<s:iterator value="resultList" id="row" status="st">
							<tr <s:if test="#st.index%2!=0">class="oddRow"</s:if>
								...
								<td align="center"><input type="checkbox" class="IDS"
									value="${row.GROUP_ID}" id="${row.GROUP_ID}"></td>
								<td>${page.firstResult + st.index+1}</td>
								<td onclick="loadView('${ctx}','${row.GROUP_ID}')"
									style="cursor: pointer;">${row.NAME}</td>
								<td onclick="loadView('${ctx}','${row.GROUP_ID}')"
									style="cursor: pointer;">${row.DEPT_ID_NAME}</td>
								<td onclick="loadView('${ctx}','${row.GROUP_ID}')"
